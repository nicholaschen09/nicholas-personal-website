        ...
        <ImageLightbox>
          <article className="w-full lg:max-w-lg lg:mx-auto">
            ...
            <p className="text-stone-500 text-sm mb-6">
              nicholas chen · march 10, 2026 · 7 min read
            </p>
            ...
            >
              <section>
                <p>
                  grpc (google remote procedure call) is a modern, open-source, high-performance rpc
                  framework that can run in any environment. it can efficiently connect services in
                  and across data centers with pluggable support for load balancing, tracing, health
                  checking and authentication. it is also applicable in last mile of distributed
                ...
                <p>
                  at its core, gRPC allows a client application to directly call a method on a
                  server application on a different machine as if it were a local object, making it
                  easier for you to create distributed applications and services. like many rpc
              ...
              <section>
                ...
                <p className="mb-6">
                  on the server side, the server implements this interface and runs a gRPC server to
                  handle client calls. on the client side, the client has a stub (referred to as
                  just a client in some languages) that provides the same methods as the server.
                </p>
                ...
                <p className="mt-6">
                  ...
                  serializing structured data (although it can be used with other data formats such
                  as json). when working with protocol buffers, the first step is to define the
                  structure for the data you want to serialize in a proto file: this is an ordinary
                  text file with a .proto extension.
                </p>
...
message HelloReply {
  string message = 1;
                ...
                <p className="mb-4">
                  once you've defined your messages and services in your .proto file, you use the
                  protocol buffer compiler protoc to generate client and server-side code in your
                  preferred language. gRPC provides plugins for many popular programming languages
                  including java, c++, python, objective-c, c#, go, ruby and node.js.
                </p>
                <p className="mb-4">
                  you use the generated server-side code to implement your service's business logic,
                  and the generated client-side code to make rpc calls to your server. gRPC handles
                  all the complexity of communicating over the network, including serialization,
                ...
                <ul className="space-y-4 text-stone-300 list-disc list-inside ml-4">
                  <li>
                    <strong className="text-white">unary RPCs</strong>: where the client sends a
                    single request to the server and gets a single response back, just like a normal
                    function call.
                  ...
                  <li>
                    <strong className="text-white">server streaming RPCs</strong>: where the client
                    sends a request to the server and gets a stream to read a sequence of messages
                    back. the client reads from the returned stream until there are no more
                    messages. gRPC guarantees message ordering within an individual rpc call.
                    <pre className="bg-stone-800/50 p-3 rounded-md overflow-x-auto text-[10px] md:text-xs text-stone-200 border border-stone-700 mt-2">
                  ...
                  <li>
                    <strong className="text-white">client streaming RPCs</strong>: where the client
                    writes a sequence of messages and sends them to the server, again using a
                    provided stream. once the client has finished writing the messages, it waits for
                    the server to read them and return its response. again gRPC guarantees message
                  ...
                  <li>
                    <strong className="text-white">bidirectional streaming RPCs</strong>: where both
                    sides send a sequence of messages using a read-write stream. the two streams
                    ...
                    then write a message, or some other combination of reads and writes. the order
                    of messages in each stream is preserved.
                    <pre className="bg-stone-800/50 p-3 rounded-md overflow-x-auto text-[10px] md:text-xs text-stone-200 border border-stone-700 mt-2">
                ...
                <p className="mb-4">
                  ...
                  complete before the rpc is terminated with a DEADLINE_EXCEEDED error. on the
                  server side, the server can query to see if a particular rpc has timed out, or how
                  much time is left to complete the rpc.
                </p>
                ...
                <p className="mb-4">
                  ...
                  the success of the call, and their conclusions may not match. this means that, for
                  example, you could have an rpc that finished successfully on the server side ("i
                  have sent all my responses!") but failed on the client side ("the responses
                  arrived after my deadline!").
                </p>
                ...
                <p className="mb-4">
                  ...
                  details) in the form of a list of key-value pairs, where the keys are strings and
                  the values are typically strings, but can be binary data.
                </p>
                ...
                <p className="mb-4">
                  ...
                  arguments to modify gRPC's default behavior, such as switching message compression
                  on or off.
                </p>
                ...
                <p className="mb-4">
                  gRPC uses a set of well-defined status codes (like OK, CANCELLED,
                  INVALID_ARGUMENT, etc.) to indicate the outcome of an rpc call. these codes are
                  similar to http status codes but are more specific to rpc systems.
                </p>
                ...
                <p className="mb-4">
                  gRPC is designed to work with various authentication mechanisms, making it easy to
                  use safely to communicate between systems. you can use alts or tls (with or
                  without google-based authentication) to encrypt and authenticate connections.
                </p>
              </section>
              ...
              <section>
                ...
                <p className="mb-6">
                  gRPC is a powerful tool for building high-performance, scalable distributed
                  systems. it's efficient, language-agnostic, and provides strong typing out of the
                  box. while it has a steeper learning curve than rest, the benefits in performance
                  and developer productivity often make it worth the investment.
                </p>
                ...
                <p className="mb-6">
                  HTTP/2 is the next-generation protocol for the web, providing significant
                  performance improvements over HTTP/1.1. gRPC uses HTTP/2 as its transport
                  protocol, allowing it to take advantage of features like multiplexing, header
                ...
                <p className="mb-6 whitespace-pre-wrap">
                  the difference between rpc (remote procedure call) and rest (representational
                  state transfer) is mostly philosophical. rpc is focused on actions—calling
                  functions on a remote server as if they were local. rest is focused on
                  resources—manipulating data objects through standard http methods (GET, POST,
                  etc.). rpc is typically more performant and easier to use for internal
                  service-to-service communication, while rest is better for public-facing apis.
                </p>
                      ...
                      {`// JSON (text, human-readable)
{
  ...
  "email": "john@example.com"
              ...
              <section>
                ...
                <p className="mb-4">
                  golang and gRPC go together like peanut butter and jelly. both were built at
                  google with high-performance networking in mind. go's built-in support for
                  concurrency via goroutines makes it perfect for handling the streaming rpcs that
                  gRPC excels at.
                </p>
                <p className="mb-4">
                  most modern cloud infrastructure and microservices are built using go and gRPC.
                  for example, the entire kubernetes ecosystem relies heavily on gRPC for
                  communication between its various components.
                </p>
                <p className="mb-4">
                  using gRPC in go is simple: you define your service in a .proto file, use the
                  protoc-gen-go and protoc-gen-go-grpc plugins to generate the go code, and then
                  implement the service interface in your go application. the result is a fast,
                  type-safe, and efficient service that's easy to maintain and scale.
                </p>
...
func main() {
    ...
    fmt.Println(r.Message)
              ...
              <section>
                ...
                <p className="mb-4">
                  while gRPC is fantastic, it's not a silver bullet for every project. here's a
                  quick guide on when you should consider using it:
                </p>
