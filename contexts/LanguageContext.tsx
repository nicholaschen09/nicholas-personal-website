...
const translations: Record<Language, Record<string, string>> = {
  en: {
    ...
    'blog.grpc.intro': "in modern software architecture, especially with microservices, the way services communicate is crucial. while REST has been the standard for a long time, gRPC has emerged as a powerful alternative for many use cases. in this post, i'll explain what gRPC is, how it works, and why you might want to use it.",
    'blog.grpc.whatIsTitle': 'what is gRPC?',
    'blog.grpc.whatIsText': "gRPC (google remote procedure call) is an open-source remote procedure call framework initially developed by google. it allows a client application to directly call a method on a server application on a different machine as if it were a local object, making it easier to create distributed applications and services. unlike REST, which is resource-oriented, gRPC is action-oriented.",
    'blog.grpc.howItWorksTitle': 'how it works',
    'blog.grpc.howItWorksText': "at a high level, gRPC allows you to define a service, specifying the methods that can be called remotely with their parameters and return types. the server implements this interface and runs a gRPC server to handle client calls. the client has a stub (referred to as just a client in some languages) that provides the same methods as the server.",
    'blog.grpc.protobufTitle': 'protocol buffers',
    'blog.grpc.protobufText': "by default, gRPC uses Protocol Buffers (Protobuf) as its Interface Definition Language (IDL) and as its underlying message interchange format. in REST, you often use JSON. with Protobuf, you define the structure of your data once in a .proto file. then, you use the protoc compiler to generate data access classes in your preferred programming language. this binary format is much lighter and faster to serialize/deserialize than JSON.",
    'blog.grpc.benefitsTitle': 'why is it good?',
    'blog.grpc.benefits.performanceTitle': 'performance',
    'blog.grpc.benefits.performanceText': "gRPC uses HTTP/2 as its transport protocol, which allows for features like multiplexing (sending multiple requests over a single connection), header compression, and binary framing. this makes gRPC much more efficient than REST over HTTP/1.1, reducing latency and bandwidth usage.",
    'blog.grpc.benefits.codeGenTitle': 'code generation',
    'blog.grpc.benefits.codeGenText': "with the .proto file serving as the contract, you can automatically generate client and server code in many languages (go, java, python, C++, node.js, etc.). this ensures that the client and server strictly adhere to the defined interface, reducing errors and saving development time.",
    'blog.grpc.benefits.streamingTitle': 'streaming',
    'blog.grpc.benefits.streamingText': "gRPC supports streaming natively. you can have server-side streaming, client-side streaming, or bidirectional streaming. this is perfect for real-time applications, large file transfers, or long-lived connections where data needs to be pushed continuously.",
    'blog.grpc.goTitle': 'gRPC with go',
    'blog.grpc.goText': "gRPC and go are a match made in heaven. since both are born out of google, the support for gRPC in go is first-class. the go ecosystem embraces gRPC for microservices due to go's concurrency model (goroutines) which handles the multiplexing nature of HTTP/2 efficiently.",
    'blog.grpc.goDetails': "to use gRPC with go, you typically define your service in a .proto file and then use the `protoc` compiler with the `protoc-gen-go` and `protoc-gen-go-grpc` plugins. this generates two files: one for the message structs and one for the service interface. on the server side, you implement the generated interface and register it with a `grpc.NewServer()`. on the client side, you use `grpc.Dial()` to connect to the server and create a client stub to make calls. the generated code is idiomatic go, making it easy to integrate into existing go projects.",
    'blog.grpc.comparisonTitle': 'gRPC vs REST',
    'blog.grpc.comparisonText': "REST is great for public APIs where human readability (JSON) and browser support are important. it's flexible and widely understood. gRPC, on the other hand, excels in internal microservices communication where low latency and high throughput are critical. it's also strongly typed, which helps in maintaining large systems.",
    'blog.grpc.conclusionTitle': 'conclusion',
    'blog.grpc.conclusionText': "gRPC is a robust framework that offers significant performance improvements and developer productivity benefits for the right use cases. if you are building a microservices architecture or need efficient communication between internal services, gRPC is definitely worth considering.",
  },
  zh: {
    // Hero section
    'hero.greeting': '嗨，我是',
    ...
    'blog.grpc.intro': "在现代软件架构中，特别是微服务中，服务之间的通信方式至关重要。虽然 REST 长期以来一直是标准，但 gRPC 已成为许多用例的强大替代方案。在这篇文章中，我将解释什么是 gRPC，它是如何工作的，以及为什么你可能想要使用它。",
    'blog.grpc.whatIsTitle': '什么是 gRPC？',
    'blog.grpc.whatIsText': "gRPC（gRPC 远程过程调用）是最初由 Google 开发的开源远程过程调用框架。它允许客户端应用程序直接调用不同机器上的服务器应用程序上的方法，就像它是本地对象一样，从而更容易创建分布式应用程序和服务。与面向资源的 REST 不同，gRPC 是面向动作的。",
    'blog.grpc.howItWorksTitle': '它是如何工作的',
    'blog.grpc.howItWorksText': "在高层面上，gRPC 允许你定义一个服务，指定可以远程调用的方法及其参数和返回类型。服务器实现此接口并运行 gRPC 服务器来处理客户端调用。客户端有一个存根（在某些语言中简称为客户端），它提供与服务器相同的方法。",
    'blog.grpc.protobufTitle': 'Protocol Buffers',
    'blog.grpc.protobufText': "默认情况下，gRPC 使用 Protocol Buffers (Protobuf) 作为其接口定义语言 (IDL) 及其底层消息交换格式。在 REST 中，你通常使用 JSON。使用 Protobuf，你在 .proto 文件中定义一次数据结构。然后，你使用 protoc 编译器用你喜欢的编程语言生成数据访问类。这种二进制格式比 JSON 更轻便，序列化/反序列化速度更快。",
    'blog.grpc.benefitsTitle': '为什么它好？',
    'blog.grpc.benefits.performanceTitle': '性能',
    'blog.grpc.benefits.performanceText': "gRPC 使用 HTTP/2 作为其传输协议，这允许诸如多路复用（通过单个连接发送多个请求）、标头压缩和二进制帧等功能。这使得 gRPC 比 HTTP/1.1 上的 REST 高效得多，减少了延迟和带宽使用。",
    'blog.grpc.benefits.codeGenTitle': '代码生成',
    'blog.grpc.benefits.codeGenText': "以 .proto 文件作为契约，你可以自动生成许多语言（Go、Java、Python、C++、Node.js 等）的客户端和服务器代码。这确保了客户端和服务器严格遵守定义的接口，减少了错误并节省了开发时间。",
    'blog.grpc.benefits.streamingTitle': '流式传输',
    'blog.grpc.benefits.streamingText': "gRPC 原生支持流式传输。你可以进行服务器端流式传输、客户端流式传输或双向流式传输。这非常适合实时应用程序、大文件传输或需要持续推送数据的长连接。",
    'blog.grpc.goTitle': 'gRPC 与 Go',
    'blog.grpc.goText': "gRPC 和 Go 是天作之合。由于两者都诞生于 Google，Go 对 gRPC 的支持是一流的。Go 生态系统拥抱 gRPC 用于微服务，因为 Go 的并发模型（goroutine）可以有效地处理 HTTP/2 的多路复用特性。",
    'blog.grpc.goDetails': "要在 Go 中使用 gRPC，通常在 .proto 文件中定义服务，然后使用带有 `protoc-gen-go` 和 `protoc-gen-go-grpc` 插件的 `protoc` 编译器。这会生成两个文件：一个用于消息结构，一个用于服务接口。在服务器端，你实现生成的接口并将其注册到 `grpc.NewServer()`。在客户端，你使用 `grpc.Dial()` 连接到服务器并创建客户端存根以进行调用。生成的代码是惯用的 Go 代码，使其易于集成到现有的 Go 项目中。",
    'blog.grpc.comparisonTitle': 'gRPC 与 REST',
    'blog.grpc.comparisonText': "REST 非常适合公共 API，其中人类可读性 (JSON) 和浏览器支持很重要。它灵活且被广泛理解。另一方面，gRPC 在内部微服务通信中表现出色，其中低延迟和高吞吐量至关重要。它也是强类型的，这有助于维护大型系统。",
    'blog.grpc.conclusionTitle': '结论',
    'blog.grpc.conclusionText': "gRPC 是一个强大的框架，为正确的用例提供了显着的性能改进和开发人员生产力优势。如果你正在构建微服务架构或需要内部服务之间的高效通信，gRPC 绝对值得考虑。",
