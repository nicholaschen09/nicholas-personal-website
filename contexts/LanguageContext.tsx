...
const translations: Record<Language, Record<string, string>> = {
  ...
  zh: {
    ...
    'ascii.title': 'rotating torus ascii render',
    'ascii.intro':
      '小实验：用等宽字符画环面，把亮度映射成字符疏密，让孔和边缘读起来像轮廓。three.js 在 react 里离屏渲染，把亮度映射到一组排序过的字符，并对每格做少量 supersample，让边缘更柔和。',
    ...
    'blog.lossless.whatIsFlacText':
      'FLAC = 自由无损音频编解码器（Free Lossless Audio Codec）。Xiph.org 开源（同 Ogg Vorbis），完全免费、无专利。常见替代：ALAC（苹果版、思路相同）、WAV/AIFF（无损但不压缩、无 LPC）、WavPack（压缩略好）。FLAC 是无损归档的事实标准：开放、支持广泛、约 50–60% 压缩率。',
